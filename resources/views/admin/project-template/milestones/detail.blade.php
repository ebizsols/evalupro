<div id="event-detail">
    ...
    <div class="modal-header">
        ...
        <h4 class="modal-title"><i class="fa fa-flag"></i> @lang('modules.projects.milestones')
            @lang('app.details')</h4>
    </div>
                        ...
                        <div class="form-group">
                            ...
                            <p>
                                ...
                                @if ($milestone->cost > 0 && $milestone->invoice_created == 0)
                                    <a href="{{ route('admin.all-invoices.convert-milestone', $milestone->id) }}"
                                        class="btn btn-xs btn-info btn-rounded m-l-15">@lang('app.create')
                                        @lang('app.invoice')</a>
                                @elseif($milestone->cost > 0 && $milestone->invoice_created == 1)
                                    <a href="{{ route('admin.all-invoices.show', $milestone->invoice_id) }}"
                                        class="btn btn-xs btn-info btn-rounded m-l-15">@lang('app.view')
                                        @lang('app.invoice')</a>
                                @endif
                            </p>
                        </div>
                                    ...
                                    @forelse ($milestone->tasks as $key=>$item)
                                        <tr>
                                            ...
                                            <td><label class="label"
                                                    style="background-color: {{ $item->board_column->label_color }}">{{ $item->board_column->column_name }}</label>
                                            </td>
                                        </tr>
