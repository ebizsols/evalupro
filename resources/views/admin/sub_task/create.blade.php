                    ...
                    <tbody>
                        @if (isset($sub_fields) && !empty($sub_fields))
                            @foreach ($sub_fields as $key => $subField)
                                <tr>
                                    <td><input type="text" name="subField[]" id="subField-{{ $key }}"
                                            class="form-control" value="{{ isset($subField) ? $subField->name : '' }}">
                                    </td>
                                </tr>
                            @endforeach
                        @endif
...
<script>
    ...
    $("#addMoreField").on("click", function() {
        subFieldTable.row.add([
            '<input type="text" name="subField[]" id="subField-' + counterSubField +
            '"  class="form-control">',
        ]).draw(false);
        counterSubField++;
    });
    ...
    $("#addMoreFeature").on("click", function() {
        ...
        featureTable.row.add([
            '<input type="text" name="fieldType[]" id="fieldType-' + counter +
            '"  class="form-control">',
            '<select name="featureType[]" class="form-control" onchange="loadFields(this.value,' +
            counter + ')" id="featureType-' + counter +
            '" autocomplete="nope"><option value="">--Select Type--</option><option value="text">Text</option><option value="select">Select Box</option><option value="textarea">Textarea</option><option value="checkbox">CheckBox</option><option value="radiobox">RadioBox</option></select>',
        ]).draw(false);
    ...
    function loadMoreOption(point) {
        ...
        tableOption.row.add([
            '<input type="text" name="selectOptions[]" id="selectOptions-' + OptionCouter +
            '"  class="form-control">',
        ]).draw(false);
        OptionCouter++;
    }
    ...
    function loadMoreCheck(index) {
        ...
        tableOption.row.add([
            '<input type="text" name="checkOptionSub[]" id="checkOptionSub-' + checkBoxCounter +
            '"  class="form-control">',
        ]).draw(false);
        checkBoxCounter++;
    }
